                ...
                if (abs(error_x) < 100 and abs(error_y) < 80):
                    ...
                    if ans=="None":
                        if dict.get("freq","status")=="status":
                            dict["freq"]=time.time()
                            # audioModule.speak("welcome to national science fest, hope you have a nice day")
...
if __name__ == '__main__':
    ...
    processing.start()
    gui.start()
    # audioModule.speak("Welcome to Tata steel tech-ex. I am Greet Bot developed by Team Robo lution under the guidance of vishal H Shah sir.")
    processing.join()
    gui.join()